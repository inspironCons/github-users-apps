...
class EmptyState @JvmOverloads constructor(
    ...
    init{
        ...
        attrs?.let { attr->
            val typedArray = context.obtainStyledAttributes(attr,R.styleable.EmptyStateLayout,0,0)
            val title = typedArray.getString(R.styleable.EmptyStateLayout_title).orEmpty()
            val desc = typedArray.getString(R.styleable.EmptyStateLayout_description)
            val drawable = typedArray.getDrawable(R.styleable.EmptyStateLayout_image)
    ...
    fun setDesctiption(desc:String){
        this.findViewById<TextView>(R.id.tv_desc).text = desc
    }
}